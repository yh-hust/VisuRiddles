...
RAW_TYPES = ["four2one_symmetry_direction", "three2three_symmetry_overall"]
...
def generate(count: int, out_dir: str | Path, assets: dict[str, Any] | None = None) -> int:
    ...
    rng = random.Random(f"attribute:group:{total}")
    for idx in range(total):
        raw_type = RAW_TYPES[idx % len(RAW_TYPES)]
        if raw_type == "three2three_symmetry_overall":
            title, choices, answer = _make_group_matrix(rng, sym_icons, non_sym_icons)
        else:
            title, choices, answer = _make_sequence(rng, sym_icons, non_sym_icons, stem_len=5)
        record = {"title": title, "choices": choices, "gt": answer, "type": raw_type}
        (out_path / f"group_{idx + 1}.json").write_text(json.dumps(record, ensure_ascii=False, indent=2), encoding="utf-8")
    return total
...
def _make_group_matrix(rng: random.Random, sym_icons: list[Path], non_sym_icons: list[Path]) -> tuple[list[list[str]], list[str], str]:
    icons = rng.sample(sym_icons, 5)
    title = [[icons[0].name, icons[1].name, icons[2].name], [icons[2].name, icons[1].name, "question_mark.png"]]
    choices = rng.sample(non_sym_icons + sym_icons, 4)
    answer_index = rng.randrange(4)
    choices[answer_index] = icons[0]
    return title, [path.name for path in choices], "ABCD"[answer_index]
